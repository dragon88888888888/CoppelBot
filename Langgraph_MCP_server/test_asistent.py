...
async def main():
    # Create GeminiConfiguration instance with default values and custom server config
    gemini_config = GeminiConfiguration(
        mcp_server_config={
        "mcpServers": {
            "github": {
                "command": "npx",
    ...
    config = RunnableConfig(
        configurable={
            ...
            "retriever_provider": gemini_config.retriever_provider,
            "mcp_server_config": gemini_config.mcp_server_config,
            "routing_query_system_prompt": gemini_config.routing_query_system_prompt,
            "routing_query_model": gemini_config.routing_query_model,
            "summarize_conversation_model": gemini_config.summarize_conversation_model,
            "routing_response_system_prompt": gemini_config.routing_response_system_prompt,
            "summarize_conversation_system_prompt": gemini_config.summarize_conversation_system_prompt,
            "routing_response_model": gemini_config.routing_response_model,
            "mcp_orchestrator_system_prompt": gemini_config.mcp_orchestrator_system_prompt,
            "mcp_orchestrator_model": gemini_config.mcp_orchestrator_model,
            "tool_refiner_model": gemini_config.tool_refiner_model,
            "tool_refiner_prompt": gemini_config.tool_refiner_prompt,
        }
    )

    # Initial state with a sample user message
    initial_state = State(
        messages=[HumanMessage(content="Dame 5 repos de github sobre langchain")],
        queries=[],
        retrieved_docs=[],
        current_mcp_server=None,
        current_tool=None
    ...
    async for event in graph.astream(initial_state, config=config):
        # Check if the event contains updated state with messages
        for node_name, state_update in event.items():
            print(f"\n=== Node: {node_name} ===")
            # Extract messages from the state update
            ...
            if messages and isinstance(messages, list):
                ...
                for m in messages:
                    if isinstance(m, BaseMessage):
                        ...
                        print(f"Non-message object: {m}")
            else:
                print("No messages in this state update")
            # Optionally print other state changes
            print("\nFull state update:", json.dumps(state_update, indent=2, default=str))
